...
def convert_arc(pt1, pt2, sagitta):
    ...
    if np.isclose(n_dist, 0):
        print('Error: Distancia entre pt1 y pt2 es demasiado pequeña.')
    ...
    if np.isclose(M11, 0):
        print('Error: El tercer punto es colineal.')
...
def rescale_frame(frame, percent=90, max_width=1280, max_height=720):

    original_height, original_width = frame.shape[:2]
    target_width = int(original_width * percent / 100)
    target_height = int(original_height * percent / 100)
...
def get_idx_coord(image, results, VISIBILITY_THRESHOLD=0.5, PRESENCE_THRESHOLD=0.5):
    idx_to_coordinates = {}
